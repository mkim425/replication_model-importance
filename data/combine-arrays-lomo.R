# Combine lomo arrays from different runs into a single list of dataframes
...
# horizon1
filenames_h1 <- list.files("data/calc-result/lomo-output", 
...
for(j in 1:10){
        ...
        list_lomo_h1[[j]] <- df 
        names(list_lomo_h1)[j] <- names(ldf[[1]][j]) 
}

# horizon2
filenames_h2 <- list.files("data/calc-result/lomo-output", 
...
for(j in 1:10){
        ...
        colnames(df) <- new_names
        list_lomo_h2[[j]] <- df
        names(list_lomo_h2)[j] <- names(ldf[[1]][j]) 
}

# horizon3
filenames_h3 <- list.files("data/calc-result/lomo-output", 
...
for(j in 1:10){
        ...
        colnames(df) <- new_names
        list_lomo_h3[[j]] <- df
        names(list_lomo_h3)[j] <- names(ldf[[1]][j]) 
}

# horizon4
filenames_h4 <- list.files("data/calc-result/lomo-output", 
...
for(j in 1:10){
        ...
        colnames(df) <- new_names
        list_lomo_h4[[j]] <- df
        names(list_lomo_h4)[j] <- names(ldf[[1]][j]) 
}
...
arrays <- c(list_lomo_h1, list_lomo_h2, list_lomo_h3, list_lomo_h4)
...
for(i in 1:40){
        ...
        lomo_df <- rbind(lomo_df, df_long)
}

# save the combined dataframe
saveRDS(lomo_df, "data/lomo_completed.rds")
